...
function discretize(seq::Sequence; sampling_params=default_sampling_params(), motion=NoMotion())
    t, Δt      = get_variable_times(
        seq;
        Δt=sampling_params["Δt"],
        Δt_rf=sampling_params["Δt_rf"],
        motion=motion,
        sample_all_blocks=get(sampling_params, "sample_all_blocks", false),
    )
    B1, Δf     = get_rfs(seq, t)
...
function default_sampling_params(sampling_params=Dict{String,Any}())
    ...
    get!(sampling_params, "sample_all_blocks", false)
